...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Namespaces'} fill={CYAN} fontFamily={CODE_FONT} fontSize={90} fontWeight={800} y={-500} opacity={0} />);
  view.add(<Txt ref={sub} text={'Virtual Clusters'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-390} opacity={0} />);

  view.add(
    <Rect ref={card} width={750} height={400} y={50} radius={24} fill={TERMINAL_BG} stroke={CYAN} lineWidth={4} opacity={0} scale={0.8}>
      <Txt text={'🏷️'} fontSize={120} y={-80} />
      <Txt text={'Isolate teams: dev, staging, prod\nResource quotas per namespace'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={30} y={50} textAlign={'center'} lineHeight={48} />
      <Txt text={'Logical separation within a cluster'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={24} y={130} textAlign={'center'} />
    </Rect>
  );
