...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Remove Nth End'} fill={ORANGE} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Remove Nth From End'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  view.add(<Txt ref={tip} text={'Two pointers with n gap\nOne pass, O(n) time 🎯'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('n=2: advance fast pointer n steps', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Move both until fast hits end', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('slow is at node before target (4)', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Skip node 4 → 1→2→3→5 ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def removeNthFromEnd(head, n):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    dummy = ListNode(0, head)', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('    fast = slow = dummy', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('    for _ in range(n + 1):', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('        fast = fast.next', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('    while fast:', 0.2));
  yield* all(codeLines[6].opacity(1, 0.12), codeLines[6].text('        slow, fast = slow.next, fast.next', 0.2));
  yield* all(codeLines[7].opacity(1, 0.12), codeLines[7].text('    slow.next = slow.next.next', 0.2));
  yield* all(codeLines[8].opacity(1, 0.12), codeLines[8].text('    return dummy.next', 0.2));
