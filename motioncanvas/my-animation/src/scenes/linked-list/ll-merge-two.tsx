...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Merge Two LL'} fill={GREEN} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Merge Two Sorted Lists'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  const arrY = -200;
    view.add(
      <Rect ref={boxes} x={startX + 0 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'1→2→4'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
    view.add(
      <Rect ref={boxes} x={startX + 1 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={' + '} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
    view.add(
      <Rect ref={boxes} x={startX + 2 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'1→3→4'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
  ...
  view.add(<Txt ref={tip} text={'O(n+m) — dummy node pattern\navoids special-casing head 🔗'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('Compare heads: 1 == 1, pick either', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('1→1→2→3→4→4', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Dummy head simplifies edge cases', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Merged! ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def mergeTwoLists(l1, l2):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    dummy = curr = ListNode()', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('    while l1 and l2:', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('        if l1.val <= l2.val:', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('            curr.next = l1; l1 = l1.next', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('        else:', 0.2));
  yield* all(codeLines[6].opacity(1, 0.12), codeLines[6].text('            curr.next = l2; l2 = l2.next', 0.2));
  yield* all(codeLines[7].opacity(1, 0.12), codeLines[7].text('        curr = curr.next', 0.2));
  yield* all(codeLines[8].opacity(1, 0.12), codeLines[8].text('    curr.next = l1 or l2', 0.2));
  yield* all(codeLines[9].opacity(1, 0.12), codeLines[9].text('    return dummy.next', 0.2));
