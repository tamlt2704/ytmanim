...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Reverse LL'} fill={GREEN} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Reverse a Linked List'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  view.add(<Txt ref={tip} text={'O(n) time, O(1) space\nSave next, flip pointer, advance 🔄'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('prev=None, curr=1', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('curr.next = prev, move forward', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('5→4→3→2→1→None', 0.3);
  yield* waitFor(0.4);
  ...
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def reverseList(head):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    prev, curr = None, head', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('    while curr:', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('        nxt = curr.next', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('        curr.next = prev', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('        prev = curr', 0.2));
  yield* all(codeLines[6].opacity(1, 0.12), codeLines[6].text('        curr = nxt', 0.2));
  yield* all(codeLines[7].opacity(1, 0.12), codeLines[7].text('    return prev', 0.2));
