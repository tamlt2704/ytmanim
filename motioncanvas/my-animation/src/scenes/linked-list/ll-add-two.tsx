...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Add Two Nums'} fill={ORANGE} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Sum as Linked Lists'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  const arrY = -200;
    view.add(
      <Rect ref={boxes} x={startX + 0 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'2→4→3'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
    view.add(
    ...
    view.add(
      <Rect ref={boxes} x={startX + 2 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'5→6→4'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
  ...
  view.add(<Txt ref={tip} text={'Process digit by digit with carry\nO(max(m,n)) time ➕'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('342 + 465 (reversed digits)', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('2+5=7, 4+6=10 carry 1, 3+4+1=8', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Result: 7→0→8 (807)', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Sum complete! ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def addTwoNumbers(l1, l2):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    dummy = curr = ListNode()', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('    carry = 0', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('    while l1 or l2 or carry:', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('        v1 = l1.val if l1 else 0', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('        v2 = l2.val if l2 else 0', 0.2));
  yield* all(codeLines[6].opacity(1, 0.12), codeLines[6].text('        carry, val = divmod(v1+v2+carry, 10)', 0.2));
  yield* all(codeLines[7].opacity(1, 0.12), codeLines[7].text('        curr.next = ListNode(val)', 0.2));
  yield* all(codeLines[8].opacity(1, 0.12), codeLines[8].text('        curr = curr.next', 0.2));
