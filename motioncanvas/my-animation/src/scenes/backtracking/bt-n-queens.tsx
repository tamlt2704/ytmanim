...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'N-Queens'} fill={RED} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Place N Queens Safely'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  const arrY = -200;
    view.add(
      <Rect ref={boxes} x={startX + 0 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'Q...'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
    view.add(
  ...
  view.add(<Txt ref={tip} text={'Sets track cols + diagonals\nO(n!) with pruning 👑'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('Place queen row by row', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Check column + both diagonals', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Backtrack if no valid column', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('n=4 → 2 solutions ✅', 0.3);
