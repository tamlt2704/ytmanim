...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Subsets'} fill={ORANGE} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'All Possible Subsets'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  view.add(<Txt ref={tip} text={'O(n·2^n) — include/skip each element\nBacktrack by popping last choice 🌿'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('[] → include or skip each element', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('[], [1], [2], [1,2], [3], [1,3]...', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Decision tree: take / skip', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('2^n = 8 subsets ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def subsets(nums):', 0.2));
  ...
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('        if i == len(nums):', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('            res.append(curr[:]); return', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('        curr.append(nums[i])', 0.2));
  yield* all(codeLines[6].opacity(1, 0.12), codeLines[6].text('        bt(i + 1, curr)  # include', 0.2));
  yield* all(codeLines[7].opacity(1, 0.12), codeLines[7].text('        curr.pop()', 0.2));
  yield* all(codeLines[8].opacity(1, 0.12), codeLines[8].text('        bt(i + 1, curr)  # skip', 0.2));
  yield* all(codeLines[9].opacity(1, 0.12), codeLines[9].text('    bt(0, []); return res', 0.2));
