...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Permutations'} fill={ORANGE} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'All Orderings'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  view.add(<Txt ref={tip} text={'O(n·n!) — try each unused element\nat each position 🔀'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('Pick each unused element next', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('[1,2,3] [1,3,2] [2,1,3] ...', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('n! = 6 permutations total', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('All 6 found ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def permute(nums):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    res = []', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('    def bt(curr, remaining):', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('        if not remaining:', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('            res.append(curr[:]); return', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('        for i in range(len(remaining)):', 0.2));
  yield* all(codeLines[6].opacity(1, 0.12), codeLines[6].text('            bt(curr + [remaining[i]],', 0.2));
  yield* all(codeLines[7].opacity(1, 0.12), codeLines[7].text('               remaining[:i] + remaining[i+1:])', 0.2));
  yield* all(codeLines[8].opacity(1, 0.12), codeLines[8].text('    bt([], nums); return res', 0.2));
