...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Unique Paths'} fill={ORANGE} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Grid Top-Left to Bottom-Right'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  const arrY = -200;
    view.add(
      <Rect ref={boxes} x={startX + 0 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'1 1 1'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
    view.add(
      <Rect ref={boxes} x={startX + 1 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'1 2 3'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
    view.add(
      <Rect ref={boxes} x={startX + 2 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'1 3 6'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
  ...
  view.add(<Txt ref={tip} text={'O(m×n) — each cell = sum of\ntop + left neighbors 🔲'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('m×n grid, only move right or down', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('dp[r][c] = dp[r-1][c] + dp[r][c-1]', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('First row and col are all 1s', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('3×3 grid → 6 paths ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def uniquePaths(m, n):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    dp = [[1] * n for _ in range(m)]', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('    for r in range(1, m):', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('        for c in range(1, n):', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('            dp[r][c] = dp[r-1][c] + dp[r][c-1]', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('    return dp[-1][-1]', 0.2));
