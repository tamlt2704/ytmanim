...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'curl'} fill={GREEN} fontFamily={CODE_FONT} fontSize={120} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Transfer Data via URLs'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={48} y={-490} opacity={0} />);
  ...
  view.add(
    <Rect ref={terminal} width={900} height={500} y={-100} radius={16} fill={TERMINAL_BG} stroke={TERMINAL_BORDER} lineWidth={2} opacity={0} scale={0.8} clip>
      ...
      <Txt ref={lines} text={''} fill={GREEN} fontFamily={CODE_FONT} fontSize={20} x={-410} y={-70} offsetX={-1} opacity={0} />
      <Txt ref={lines} text={''} fill={GREEN} fontFamily={CODE_FONT} fontSize={20} x={-410} y={-25} offsetX={-1} opacity={0} />
      <Txt ref={lines} text={''} fill={GREEN} fontFamily={CODE_FONT} fontSize={20} x={-410} y={20} offsetX={-1} opacity={0} />
    </Rect>
  );
  view.add(<Txt ref={tip} text={'-s silent, -o output file\n-X to set HTTP method 🌐'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={30} y={550} textAlign={'center'} lineHeight={50} opacity={0} />);
  ...
  const cmd = '$ curl -s https://api.example.com';
  for (let i = 0; i <= cmd.length; i++) { lines[0].text(cmd.slice(0, i) + '▌'); yield* waitFor(0.04); }
  lines[0].text(cmd);
  yield* waitFor(0.3);
  yield* all(lines[1].opacity(1, 0.3), lines[1].text('{"status": "ok",', 0.3));
  yield* all(lines[2].opacity(1, 0.3), lines[2].text(' "users": 1542,', 0.3));
  yield* all(lines[3].opacity(1, 0.3), lines[3].text(' "version": "2.1.0"}', 0.3));
