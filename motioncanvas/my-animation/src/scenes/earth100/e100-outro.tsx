...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={recap} text={'If Earth had 100 people 🌍'} fill={ACCENT_COLOR} fontFamily={CODE_FONT} fontSize={32} fontWeight={700} y={120} opacity={0} />);

  const topics = createRef<Txt>();
  view.add(<Txt ref={topics} text={'🌍 Continents  👶 Age  🗣️ Language\n🙏 Religion  📚 Literacy  💧 Water\n💰 Wealth — 1 owns more than 50'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={22} y={270} textAlign={'center'} lineHeight={42} opacity={0} />);

  const follow = createRef<Txt>();
  view.add(<Txt ref={follow} text={'More perspective shifts coming! 🔥'} fill={ORANGE} fontFamily={TITLE_FONT} fontSize={32} y={420} opacity={0} />);

  const handle = createRef<Txt>();
  view.add(<Txt ref={handle} text={'@it4life101'} fill={PURPLE} fontFamily={CODE_FONT} fontSize={44} fontWeight={800} y={510} opacity={0} />);
