...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'docker stop'} fill={RED} fontFamily={CODE_FONT} fontSize={95} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Stop a Container'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={48} y={-490} opacity={0} />);
  ...
  view.add(
    <Rect ref={terminal} width={900} height={500} y={-100} radius={16} fill={TERMINAL_BG} stroke={TERMINAL_BORDER} lineWidth={2} opacity={0} scale={0.8} clip>
      ...
      <Txt ref={lines} text={''} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={28} x={-410} y={-130} offsetX={-1} opacity={0} />
      <Txt ref={lines} text={''} fill={RED} fontFamily={CODE_FONT} fontSize={26} x={-410} y={-60} offsetX={-1} opacity={0} />
      <Txt ref={lines} text={''} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={28} x={-410} y={20} offsetX={-1} opacity={0} />
      <Txt ref={lines} text={''} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={24} x={-410} y={80} offsetX={-1} opacity={0} />
    </Rect>
  );

  view.add(<Txt ref={tip} text={'Sends SIGTERM, then SIGKILL\nafter 10s grace period ⏱️'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={30} y={550} textAlign={'center'} lineHeight={50} opacity={0} />);
  ...
  const cmd = '$ docker stop nginx';
  for (let i = 0; i <= cmd.length; i++) { lines[0].text(cmd.slice(0, i) + '▌'); yield* waitFor(0.05); }
  lines[0].text(cmd);
  yield* waitFor(0.5);

  yield* all(lines[1].opacity(1, 0.3), lines[1].text('nginx — Stopped ✓', 0.5));
  yield* waitFor(0.3);

  yield* lines[2].opacity(1, 0.2);
  const cmd2 = '$ docker kill redis';
  for (let i = 0; i <= cmd2.length; i++) { lines[2].text(cmd2.slice(0, i) + '▌'); yield* waitFor(0.05); }
  lines[2].text(cmd2);
  yield* all(lines[3].opacity(1, 0.3), lines[3].text('Force kill — no grace period!', 0.4));
