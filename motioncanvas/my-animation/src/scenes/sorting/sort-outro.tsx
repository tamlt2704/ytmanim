...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={recap} text={'5 sorting algorithms visualized!'} fill={ACCENT_COLOR} fontFamily={CODE_FONT} fontSize={32} fontWeight={700} y={130} opacity={0} />);

  const topics = createRef<Txt>();
  view.add(<Txt ref={topics} text={'🫧 Bubble  🎯 Selection  🃏 Insertion\n🔗 Merge  ⚡ Quick'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={26} y={280} textAlign={'center'} lineHeight={46} opacity={0} />);

  const follow = createRef<Txt>();
  view.add(<Txt ref={follow} text={'More algorithms coming! 🔥'} fill={ORANGE} fontFamily={TITLE_FONT} fontSize={36} y={420} opacity={0} />);
