...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Contains Dup'} fill={GREEN} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Any Value Appears Twice?'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  view.add(<Txt ref={tip} text={'O(n) time, O(n) space\nSet lookup is O(1) average 🔍'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('Add to set: {1}', 0.3);
  yield* waitFor(0.3);
  yield* statusTxt().text('Add to set: {1, 2}', 0.3);
  yield* waitFor(0.3);
  yield* statusTxt().text('Add to set: {1, 2, 3}', 0.3);
  yield* waitFor(0.3);
  yield* statusTxt().text('1 already in set! → True ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def containsDuplicate(nums):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    seen = set()', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('    for n in nums:', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('        if n in seen: return True', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('        seen.add(n)', 0.2));
