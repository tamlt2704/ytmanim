...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Min Arrows'} fill={ORANGE} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Burst Overlapping Balloons'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  const arrY = -200;
    view.add(
      <Rect ref={boxes} x={startX + 0 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'[10,16]'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
    view.add(
      <Rect ref={boxes} x={startX + 1 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'[2,8]'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
    view.add(
      <Rect ref={boxes} x={startX + 2 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'[1,6]'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
    view.add(
      <Rect ref={boxes} x={startX + 3 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'[7,12]'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
  ...
  view.add(<Txt ref={tip} text={'O(n log n) — greedy by end point\nSame as non-overlapping intervals 🏹'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('Sort by end point', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Shoot at end of first balloon', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Skip all balloons hit by this arrow', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('2 arrows needed ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def findMinArrowShots(points):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    points.sort(key=lambda x: x[1])', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('    arrows = 1', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('    end = points[0][1]', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('    for s, e in points[1:]:', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('        if s > end:', 0.2));
  yield* all(codeLines[6].opacity(1, 0.12), codeLines[6].text('            arrows += 1', 0.2));
  yield* all(codeLines[7].opacity(1, 0.12), codeLines[7].text('            end = e', 0.2));
  yield* all(codeLines[8].opacity(1, 0.12), codeLines[8].text('    return arrows', 0.2));
