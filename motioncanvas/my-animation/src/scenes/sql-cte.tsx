...
export default makeScene2D(function* (view) {
  ...
  yield* all(lines[0].opacity(1, 0.3), lines[0].text('WITH active AS (|  SELECT * FROM users|  WHERE active = true|) SELECT * FROM active;', 0.4));
  yield* all(lines[1].opacity(1, 0.3), lines[1].text('Named temporary result set\nfor cleaner queries ✨', 0.4));
  yield* all(lines[2].opacity(1, 0.3), lines[2].text('Recursive CTEs for trees!', 0.4));
