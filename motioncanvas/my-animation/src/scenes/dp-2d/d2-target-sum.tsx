...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Target Sum'} fill={ORANGE} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Assign +/- to Reach Target'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  view.add(<Txt ref={tip} text={'Reduce to subset sum: (total+target)/2\nO(n×sum) knapsack variant 🎯'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('target=3, assign + or - to each num', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Reduce to subset sum problem', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('dp[i][s] = ways to reach sum s using i nums', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('5 ways to make 3 ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def findTargetSumWays(nums, target):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    total = sum(nums)', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('    if (total + target) % 2: return 0', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('    goal = (total + target) // 2', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('    dp = [0] * (goal + 1)', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('    dp[0] = 1', 0.2));
  yield* all(codeLines[6].opacity(1, 0.12), codeLines[6].text('    for n in nums:', 0.2));
  yield* all(codeLines[7].opacity(1, 0.12), codeLines[7].text('        for j in range(goal, n - 1, -1):', 0.2));
  yield* all(codeLines[8].opacity(1, 0.12), codeLines[8].text('            dp[j] += dp[j - n]', 0.2));
  yield* all(codeLines[9].opacity(1, 0.12), codeLines[9].text('    return dp[goal]', 0.2));
