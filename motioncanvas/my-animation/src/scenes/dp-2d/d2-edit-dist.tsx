...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Edit Distance'} fill={ORANGE} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Min Ops to Transform'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  const startX = -((2 - 1) * (boxSize + gap)) / 2;
  const arrY = -200;
    view.add(
      <Rect ref={boxes} x={startX + 0 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'h o r s e'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
    view.add(
      <Rect ref={boxes} x={startX + 1 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'r o s'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
  ...
  view.add(<Txt ref={tip} text={'O(m×n) — Levenshtein distance\nBase cases: empty string transforms ✏️'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('3 operations: insert, delete, replace', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Match: dp[i-1][j-1] (free)', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Else: 1 + min(insert, delete, replace)', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('horse → ros = 3 ops ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def minDistance(w1, w2):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    m, n = len(w1), len(w2)', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('    dp = [[0]*(n+1) for _ in range(m+1)]', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('    for i in range(m+1): dp[i][0] = i', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('    for j in range(n+1): dp[0][j] = j', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('    for i in range(1, m+1):', 0.2));
  yield* all(codeLines[6].opacity(1, 0.12), codeLines[6].text('        for j in range(1, n+1):', 0.2));
  yield* all(codeLines[7].opacity(1, 0.12), codeLines[7].text('            if w1[i-1] == w2[j-1]:', 0.2));
  yield* all(codeLines[8].opacity(1, 0.12), codeLines[8].text('                dp[i][j] = dp[i-1][j-1]', 0.2));
  yield* all(codeLines[9].opacity(1, 0.12), codeLines[9].text('            else: dp[i][j] = 1 + min(', 0.2));
  yield* all(codeLines[10].opacity(1, 0.12), codeLines[10].text('                dp[i-1][j], dp[i][j-1], dp[i-1][j-1])', 0.2));
