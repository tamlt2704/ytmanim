...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Burst Balloons'} fill={RED} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Max Coins from Bursting'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  view.add(<Txt ref={tip} text={'O(n³) interval DP\nThink: which balloon bursts LAST? 🎈'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('dp[i][j] = max coins bursting i..j', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Pick LAST balloon to burst in range', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('coins = nums[i-1]*nums[k]*nums[j+1]', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Max = 167 ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def maxCoins(nums):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    nums = [1] + nums + [1]', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('    n = len(nums)', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('    dp = [[0]*n for _ in range(n)]', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('    for length in range(2, n):', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('        for i in range(n - length):', 0.2));
  yield* all(codeLines[6].opacity(1, 0.12), codeLines[6].text('            j = i + length', 0.2));
  yield* all(codeLines[7].opacity(1, 0.12), codeLines[7].text('            for k in range(i+1, j):', 0.2));
  yield* all(codeLines[8].opacity(1, 0.12), codeLines[8].text('                coins = nums[i]*nums[k]*nums[j]', 0.2));
  yield* all(codeLines[9].opacity(1, 0.12), codeLines[9].text('                dp[i][j] = max(dp[i][j],', 0.2));
  yield* all(codeLines[10].opacity(1, 0.12), codeLines[10].text('                    dp[i][k] + coins + dp[k][j])', 0.2));
