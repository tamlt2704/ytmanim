...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Single Number'} fill={GREEN} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Find the Unpaired One'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  view.add(<Txt ref={tip} text={'O(n) time, O(1) space\nXOR cancels pairs, leaves single ⊕'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('XOR: a ^ a = 0, a ^ 0 = a', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('4^1^2^1^2 = 4^(1^1)^(2^2)', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('= 4 ^ 0 ^ 0 = 4', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Single number = 4 ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def singleNumber(nums):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    res = 0', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('    for n in nums:', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('        res ^= n', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('    return res', 0.2));
