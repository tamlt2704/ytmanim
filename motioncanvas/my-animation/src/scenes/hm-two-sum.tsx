...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Two Sum'} fill={GREEN} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Find Pair = Target'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  view.add(<Txt ref={tip} text={'O(n) time, O(n) space\nOne-pass hashmap lookup 🎯'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('target = 9, map = {}', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('2 → need 7, store {2:0}', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('7 → found 7 in map! [0, 1] ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def twoSum(nums, target):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    seen = {}', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('    for i, n in enumerate(nums):', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('        diff = target - n', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('        if diff in seen:', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('            return [seen[diff], i]', 0.2));
  yield* all(codeLines[6].opacity(1, 0.12), codeLines[6].text('        seen[n] = i', 0.2));
