...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'WHERE'} fill={ORANGE} fontFamily={CODE_FONT} fontSize={110} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Filter Results'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={48} y={-490} opacity={0} />);
  ...
  view.add(
    <Rect ref={terminal} width={900} height={500} y={-100} radius={16} fill={TERMINAL_BG} stroke={TERMINAL_BORDER} lineWidth={2} opacity={0} scale={0.8} clip>
      ...
      <Txt ref={lines} text={''} fill={ORANGE} fontFamily={CODE_FONT} fontSize={24} x={-410} y={-90} offsetX={-1} opacity={0} />
      <Txt ref={lines} text={''} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={20} x={-410} y={-30} offsetX={-1} opacity={0} />
      <Txt ref={lines} text={''} fill={GREEN} fontFamily={CODE_FONT} fontSize={20} x={-410} y={10} offsetX={-1} opacity={0} />
    </Rect>
  );
  view.add(<Txt ref={tip} text={'Use AND, OR, IN, BETWEEN\nLIKE for pattern matching 🎯'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={30} y={550} textAlign={'center'} lineHeight={50} opacity={0} />);
  ...
  yield* all(lines[0].opacity(1, 0.3), lines[0].text('SELECT * FROM users', 0.4));
  yield* all(lines[1].opacity(1, 0.3), lines[1].text('WHERE age > 21 AND active = true;', 0.5));
  yield* waitFor(0.3);
  yield* all(lines[2].opacity(1, 0.3), lines[2].text('name   | age | active', 0.3));
  yield* all(lines[3].opacity(1, 0.3), lines[3].text('Alice  | 28  | true', 0.3));
