...
export default makeScene2D(function* (view) {
  ...
  view.add(<Rect ref={badge} x={0} y={-750} width={300} height={100} radius={50} fill={ACCENT_COLOR} opacity={0} scale={0}><Txt text={'#10'} fill={'#fff'} fontFamily={TITLE_FONT} fontSize={56} fontWeight={800} /></Rect>);
  view.add(<Txt ref={title} text={'CTE'} fill={ACCENT_COLOR} fontFamily={CODE_FONT} fontSize={100} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Common Table Expr'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={48} y={-490} opacity={0} />);
  ...
  view.add(<Txt ref={tip} text={'Named temporary result set\nfor cleaner queries ✨'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={30} y={550} textAlign={'center'} lineHeight={50} opacity={0} />);
  ...
  yield* all(lines[0].opacity(1, 0.3), lines[0].text('WITH active AS (', 0.4));
  yield* all(lines[1].opacity(1, 0.3), lines[1].text('  SELECT * FROM users', 0.4));
  yield* all(lines[2].opacity(1, 0.3), lines[2].text('  WHERE active = true)', 0.4));
