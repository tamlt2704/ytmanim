...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={recap} text={'Your eyes just lied to you! 🌀'} fill={ACCENT_COLOR} fontFamily={CODE_FONT} fontSize={32} fontWeight={700} y={130} opacity={0} />);

  const topics = createRef<Txt>();
  view.add(<Txt ref={topics} text={'👁️ Peripheral drift illusion\n🧠 Different contrast = fake motion\n🔄 Color order controls direction'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={24} y={290} textAlign={'center'} lineHeight={44} opacity={0} />);

  const follow = createRef<Txt>();
  view.add(<Txt ref={follow} text={'More illusions coming! 🔥'} fill={ORANGE} fontFamily={TITLE_FONT} fontSize={36} y={450} opacity={0} />);

  const handle = createRef<Txt>();
  view.add(<Txt ref={handle} text={'@it4life101'} fill={PURPLE} fontFamily={CODE_FONT} fontSize={44} fontWeight={800} y={540} opacity={0} />);
