...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Reverse Integer'} fill={ORANGE} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Reverse Digits of Int'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  view.add(<Txt ref={tip} text={'O(log n) digits — pop and push\nCheck 32-bit bounds before return 🔢'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('Pop last digit: x % 10', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Push to result: res * 10 + digit', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Check 32-bit overflow each step', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('123 → 321 ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def reverse(x):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    res = 0', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('    sign = -1 if x < 0 else 1', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('    x = abs(x)', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('    while x:', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('        res = res * 10 + x % 10', 0.2));
  yield* all(codeLines[6].opacity(1, 0.12), codeLines[6].text('        x //= 10', 0.2));
  yield* all(codeLines[7].opacity(1, 0.12), codeLines[7].text('    res *= sign', 0.2));
  yield* all(codeLines[8].opacity(1, 0.12), codeLines[8].text('    return res if -2**31 <= res < 2**31 else 0', 0.2));
