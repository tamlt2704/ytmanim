...
export default makeScene2D(function* (view) {
  ...
  view.add(<Rect ref={badge} x={0} y={-800} width={300} height={100} radius={50} fill={ACCENT_COLOR} opacity={0} scale={0}><Txt text={'#5'} fill={'#fff'} fontFamily={TITLE_FONT} fontSize={56} fontWeight={800} /></Rect>);

  const title = createRef<Txt>();
  view.add(<Txt ref={title} text={'Fermi Paradox'} fill={ACCENT_COLOR} fontFamily={CODE_FONT} fontSize={85} fontWeight={800} y={-680} opacity={0} />);
  ...
  view.add(<Txt ref={r0} text={''} fill={ACCENT_COLOR} fontFamily={CODE_FONT} fontSize={52} fontWeight={800} y={-150} opacity={0} />);

  const tip = createRef<Txt>();
  view.add(<Txt ref={tip} text={'Great Filter? Zoo hypothesis?\nWe truly do not know yet'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={600} textAlign={'center'} lineHeight={46} opacity={0} />);

  yield* s0().opacity(1, 0.2);
  yield* s0().text('Billions of stars, billions of years.', 0.3);
  yield* waitFor(0.3);
  yield* s1().opacity(1, 0.2);
  yield* s1().text('The universe should be teeming with life.', 0.3);
  yield* waitFor(0.3);

  yield* fadeInUp(r0(), 20, 0.3);
  yield* r0().text('So... where is everybody?', 0.3);
