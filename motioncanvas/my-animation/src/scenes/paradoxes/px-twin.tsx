...
export default makeScene2D(function* (view) {
  ...
  view.add(<Rect ref={badge} x={0} y={-800} width={300} height={100} radius={50} fill={ACCENT_COLOR} opacity={0} scale={0}><Txt text={'#6'} fill={'#fff'} fontFamily={TITLE_FONT} fontSize={56} fontWeight={800} /></Rect>);

  const title = createRef<Txt>();
  view.add(<Txt ref={title} text={'Twin Paradox'} fill={ORANGE} fontFamily={CODE_FONT} fontSize={85} fontWeight={800} y={-680} opacity={0} />);
  ...
  view.add(<Txt ref={r0} text={''} fill={ORANGE} fontFamily={CODE_FONT} fontSize={44} fontWeight={800} y={-150} opacity={0} />);
  const r1 = createRef<Txt>();
  view.add(<Txt ref={r1} text={''} fill={RED} fontFamily={CODE_FONT} fontSize={40} fontWeight={800} y={-80} opacity={0} />);

  const tip = createRef<Txt>();
  view.add(<Txt ref={tip} text={'Acceleration breaks the symmetry\nEinstein was right: time is relative'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={600} textAlign={'center'} lineHeight={46} opacity={0} />);

  yield* s0().opacity(1, 0.2);
  yield* s0().text('One twin flies near light speed.', 0.3);
  yield* waitFor(0.3);
  yield* s1().opacity(1, 0.2);
  yield* s1().text('Returns younger than the other.', 0.3);
  yield* waitFor(0.3);

  yield* fadeInUp(r0(), 20, 0.3);
  yield* r0().text('Time slowed down for the traveler!', 0.3);
  yield* pulse(r0() as any, 1.15, 0.3);
  yield* fadeInUp(r1(), 20, 0.3);
  yield* r1().text('Both see the other aging slower?', 0.3);
