...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Non-Overlap'} fill={ORANGE} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Min Removals for No Overlap'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  const arrY = -200;
    ...
    view.add(
      <Rect ref={boxes} x={startX + 2 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'[3,4]'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
    view.add(
  ...
  view.add(<Txt ref={tip} text={'O(n log n) — sort by end time\nGreedy: earliest end = most room 🎯'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('Sort by END time (greedy)', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Keep interval that ends earliest', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Remove [1,3] — it overlaps [1,2]', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Remove 1 interval ✅', 0.3);
  ...
  yield* all(codeLines[8].opacity(1, 0.12), codeLines[8].text('            count += 1', 0.2));
  yield* all(codeLines[9].opacity(1, 0.12), codeLines[9].text('    return count', 0.2));
