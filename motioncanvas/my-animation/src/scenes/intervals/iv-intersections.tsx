...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Intersections'} fill={ORANGE} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Two Interval List Overlap'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  const arrY = -200;
    view.add(
      <Rect ref={boxes} x={startX + 0 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'A:[0,2]'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
    view.add(
      <Rect ref={boxes} x={startX + 1 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'[5,10]'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
    view.add(
      <Rect ref={boxes} x={startX + 2 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'B:[1,5]'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
    view.add(
      <Rect ref={boxes} x={startX + 3 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'[8,12]'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
  ...
  view.add(<Txt ref={tip} text={'O(m+n) two pointers\nAdvance whichever ends first 🔀'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('Two pointers, one per list', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Overlap = [max(starts), min(ends)]', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Advance pointer with smaller end', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('[[1,2],[5,5],[8,10]] ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def intervalIntersection(A, B):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    res = []; i = j = 0', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('    while i < len(A) and j < len(B):', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('        lo = max(A[i][0], B[j][0])', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('        hi = min(A[i][1], B[j][1])', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('        if lo <= hi:', 0.2));
  yield* all(codeLines[6].opacity(1, 0.12), codeLines[6].text('            res.append([lo, hi])', 0.2));
  yield* all(codeLines[7].opacity(1, 0.12), codeLines[7].text('        if A[i][1] < B[j][1]: i += 1', 0.2));
  yield* all(codeLines[8].opacity(1, 0.12), codeLines[8].text('        else: j += 1', 0.2));
  yield* all(codeLines[9].opacity(1, 0.12), codeLines[9].text('    return res', 0.2));
