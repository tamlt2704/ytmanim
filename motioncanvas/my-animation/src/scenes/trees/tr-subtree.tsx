...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Subtree'} fill={GREEN} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Is t a Subtree of s?'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  const arrY = -200;
    ...
    view.add(
      <Rect ref={boxes} x={startX + 1 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'4  5'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
    view.add(
      <Rect ref={boxes} x={startX + 2 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'1 2'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
  ...
  view.add(<Txt ref={tip} text={'O(m·n) — isSameTree at each node\nCan optimize with serialization 🌲'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('For each node in s, check isSameTree', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('s.left (4) matches t? Check...', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('4→(1,2) == t → match!', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Is subtree! ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def isSubtree(root, sub):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    if not root: return False', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('    if isSameTree(root, sub):', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('        return True', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('    return (isSubtree(root.left, sub)', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('        or isSubtree(root.right, sub))', 0.2));
