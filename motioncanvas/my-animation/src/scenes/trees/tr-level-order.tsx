...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Level Order'} fill={ORANGE} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'BFS Layer by Layer'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  const arrY = -200;
    ...
    view.add(
      <Rect ref={boxes} x={startX + 1 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'9  20'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
    view.add(
      <Rect ref={boxes} x={startX + 2 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'   15 7'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
  ...
  view.add(<Txt ref={tip} text={'BFS with queue — process level size\nO(n) time 📊'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('Queue: [3] → level 0', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Queue: [9, 20] → level 1', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Queue: [15, 7] → level 2', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('[[3],[9,20],[15,7]] ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def levelOrder(root):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    if not root: return []', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('    res, q = [], deque([root])', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('    while q:', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('        level = []', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('        for _ in range(len(q)):', 0.2));
  yield* all(codeLines[6].opacity(1, 0.12), codeLines[6].text('            node = q.popleft()', 0.2));
  yield* all(codeLines[7].opacity(1, 0.12), codeLines[7].text('            level.append(node.val)', 0.2));
  yield* all(codeLines[8].opacity(1, 0.12), codeLines[8].text('            if node.left: q.append(node.left)', 0.2));
  yield* all(codeLines[9].opacity(1, 0.12), codeLines[9].text('            if node.right: q.append(node.right)', 0.2));
  yield* all(codeLines[10].opacity(1, 0.12), codeLines[10].text('        res.append(level)', 0.2));
