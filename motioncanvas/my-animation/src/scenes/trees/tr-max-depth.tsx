...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Max Depth'} fill={GREEN} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Height of Binary Tree'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  view.add(<Txt ref={tip} text={'O(n) DFS recursion\nBase case: null → 0 🌳'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('Depth = 1 + max(left, right)', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Left subtree depth = 1', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Right subtree depth = 2', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Max depth = 3 ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def maxDepth(root):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    if not root: return 0', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('    left = maxDepth(root.left)', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('    right = maxDepth(root.right)', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('    return 1 + max(left, right)', 0.2));
