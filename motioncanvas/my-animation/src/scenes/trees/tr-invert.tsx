...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Invert Tree'} fill={GREEN} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Mirror a Binary Tree'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  const arrY = -200;
    ...
    view.add(
      <Rect ref={boxes} x={startX + 1 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'2  7'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
    view.add(
      <Rect ref={boxes} x={startX + 2 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'1 3 6 9'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
  ...
  view.add(<Txt ref={tip} text={'O(n) — visit every node once\nSwap children at each level 🪞'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('Swap left and right children', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Recurse on each subtree', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('4→(7,2)→(9,6,3,1)', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Inverted! ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def invertTree(root):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    if not root: return None', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('    root.left, root.right = (', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('        root.right, root.left)', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('    invertTree(root.left)', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('    invertTree(root.right)', 0.2));
  yield* all(codeLines[6].opacity(1, 0.12), codeLines[6].text('    return root', 0.2));
