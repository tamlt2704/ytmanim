...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Valid BST'} fill={ORANGE} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Is It a Valid BST?'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  const arrY = -200;
    ...
    view.add(
      <Rect ref={boxes} x={startX + 1 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'1  7'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
    view.add(
      <Rect ref={boxes} x={startX + 2 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'   6 8'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
  ...
  view.add(<Txt ref={tip} text={'O(n) — pass valid range down\nNot just left < root < right! 🔍'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('Each node has a valid range (lo, hi)', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('root=5: (-inf, inf) ✓', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('left=1: (-inf, 5) ✓  right=7: (5, inf) ✓', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('All nodes valid → True ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def isValidBST(root, lo=-inf, hi=inf):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    if not root: return True', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('    if not (lo < root.val < hi):', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('        return False', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('    return (', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('        isValidBST(root.left, lo, root.val)', 0.2));
  yield* all(codeLines[6].opacity(1, 0.12), codeLines[6].text('        and isValidBST(root.right, root.val, hi)', 0.2));
  yield* all(codeLines[7].opacity(1, 0.12), codeLines[7].text('    )', 0.2));
