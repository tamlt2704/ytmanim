...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Same Tree'} fill={GREEN} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Are Two Trees Identical?'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  const arrY = -200;
    ...
    view.add(
      <Rect ref={boxes} x={startX + 1 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'2  3'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
  ...
  view.add(<Txt ref={tip} text={'O(n) — check val + recurse both sides\nThree base cases to handle 🌿'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('Compare root values', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Recurse: same left AND same right', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Both null → True (base case)', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Trees are identical ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def isSameTree(p, q):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    if not p and not q: return True', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('    if not p or not q: return False', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('    if p.val != q.val: return False', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('    return (isSameTree(p.left, q.left)', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('        and isSameTree(p.right, q.right))', 0.2));
