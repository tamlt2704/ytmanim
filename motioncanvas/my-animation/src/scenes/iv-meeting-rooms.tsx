...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Meeting Rooms'} fill={GREEN} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Can Attend All?'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  view.add(<Txt ref={tip} text={'O(n log n) — sort then check\nadjacent pairs for overlap 📅'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* waitFor(0.4);
  yield* statusTxt().text('[0,30] and [5,10] overlap!', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('5 < 30 → conflict', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Cannot attend all → False ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def canAttendMeetings(intervals):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    intervals.sort()', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('    for i in range(1, len(intervals)):', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('        if intervals[i][0] < intervals[i-1][1]:', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('            return False', 0.2));
