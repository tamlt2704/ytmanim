...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Word Break'} fill={ORANGE} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Can String Be Segmented?'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  const arrY = -200;
    view.add(
      <Rect ref={boxes} x={startX + 0 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'l'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={28} fontWeight={800} />
      </Rect>
    );
    view.add(
      <Rect ref={boxes} x={startX + 1 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'e'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={28} fontWeight={800} />
      </Rect>
    );
    view.add(
    ...
    view.add(
      <Rect ref={boxes} x={startX + 3 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'t'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={28} fontWeight={800} />
      </Rect>
    );
    view.add(
      <Rect ref={boxes} x={startX + 4 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'c'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={28} fontWeight={800} />
      </Rect>
    );
    view.add(
      <Rect ref={boxes} x={startX + 5 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'o'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={28} fontWeight={800} />
      </Rect>
    );
    view.add(
      <Rect ref={boxes} x={startX + 6 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'d'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={28} fontWeight={800} />
      </Rect>
    );
    view.add(
  ...
  view.add(<Txt ref={tip} text={'O(n²·k) — try every split point\ndp[j] valid + s[j:i] in dict 📖'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('dict = [leet, code]', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('dp[i] = can s[:i] be segmented?', 0.3);
  yield* waitFor(0.4);
  ...
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def wordBreak(s, wordDict):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    words = set(wordDict)', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('    dp = [False] * (len(s) + 1)', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('    dp[0] = True', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('    for i in range(1, len(s) + 1):', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('        for j in range(i):', 0.2));
  yield* all(codeLines[6].opacity(1, 0.12), codeLines[6].text('            if dp[j] and s[j:i] in words:', 0.2));
  yield* all(codeLines[7].opacity(1, 0.12), codeLines[7].text('                dp[i] = True; break', 0.2));
  yield* all(codeLines[8].opacity(1, 0.12), codeLines[8].text('    return dp[-1]', 0.2));
