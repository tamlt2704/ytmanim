...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Valid Parens'} fill={GREEN} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Matching Brackets'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  view.add(<Txt ref={tip} text={'O(n) — push open, pop on close\nStack must be empty at end 🔗'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('( → push to stack', 0.3);
  yield* waitFor(0.3);
  yield* statusTxt().text(') → pop, matches ( ✓', 0.3);
  yield* waitFor(0.3);
  yield* statusTxt().text('Stack empty at end → valid', 0.3);
  yield* waitFor(0.4);
  ...
  yield* all(codeLines[6].opacity(1, 0.12), codeLines[6].text('            return False', 0.2));
  yield* all(codeLines[7].opacity(1, 0.12), codeLines[7].text('    return not stack', 0.2));
