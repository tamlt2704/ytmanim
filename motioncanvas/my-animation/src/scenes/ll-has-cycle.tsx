...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Has Cycle'} fill={GREEN} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Detect Cycle in LL'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  yield* statusTxt().text('slow moves 1 step, fast moves 2', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('If cycle exists, they MUST meet', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('slow=0, fast=0 → they meet!', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Cycle detected! ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def hasCycle(head):', 0.2));
  ...
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('        slow = slow.next', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('        fast = fast.next.next', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('        if slow == fast:', 0.2));
  yield* all(codeLines[6].opacity(1, 0.12), codeLines[6].text('            return True', 0.2));
