...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={recap} text={'70 years of programming!'} fill={ACCENT_COLOR} fontFamily={CODE_FONT} fontSize={32} fontWeight={700} y={120} opacity={0} />);

  const topics = createRef<Txt>();
  view.add(<Txt ref={topics} text={'🔬 Fortran → 🐍 Python → 🦀 Rust\n1957                                    2025\nWhat language will be next? 🤔'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={24} y={270} textAlign={'center'} lineHeight={44} opacity={0} />);

  const follow = createRef<Txt>();
  view.add(<Txt ref={follow} text={'More tech history coming! 🔥'} fill={ORANGE} fontFamily={TITLE_FONT} fontSize={36} y={430} opacity={0} />);
