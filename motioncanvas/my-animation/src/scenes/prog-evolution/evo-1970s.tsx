...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={era} text={'1970s — 1980s'} fill={ACCENT_COLOR} fontFamily={TITLE_FONT} fontSize={72} fontWeight={900} y={-820} letterSpacing={6} opacity={0} />);
  yield* fadeInUp(era(), 30, 0.4);

  const subtitle = createRef<Txt>();
  view.add(<Txt ref={subtitle} text={'The Foundations ⚙️'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={32} y={-740} opacity={0} />);
  ...
  const langs = [
    {name: 'C', year: '1972', emoji: '⚡', color: ACCENT_COLOR, desc: 'The mother of modern languages\nUnix, Linux, Windows kernels'},
    {name: 'SQL', year: '1974', emoji: '🗄️', color: GREEN, desc: 'Database query language\nStill #1 for data everywhere'},
    {name: 'C++', year: '1979', emoji: '🎮', color: PURPLE, desc: 'C with classes & OOP\nGames, browsers, OS kernels'},
    {name: 'Objective-C', year: '1984', emoji: '🍎', color: ORANGE, desc: 'Apple ecosystem for 30 years\nPowered every iPhone app until Swift'},
    {name: 'Perl', year: '1987', emoji: '🐪', color: RED, desc: 'Text processing powerhouse\nThe "duct tape of the internet"'},
  ];
