...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Meeting Rooms II'} fill={ORANGE} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Min Rooms Needed'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  const arrY = -200;
    view.add(
      <Rect ref={boxes} x={startX + 0 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'[0,30]'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
    view.add(
    ...
    view.add(
      <Rect ref={boxes} x={startX + 2 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'[15,20]'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
  ...
  view.add(<Txt ref={tip} text={'O(n log n) — sweep line approach\nOr use min-heap of end times 🏢'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('Separate starts and ends, sort both', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Sweep: start → +1 room, end → -1 room', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Track max concurrent meetings', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Need 2 rooms ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def minMeetingRooms(intervals):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    starts = sorted(i[0] for i in intervals)', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('    ends = sorted(i[1] for i in intervals)', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('    rooms = ep = 0', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('    for s in starts:', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('        if s < ends[ep]:', 0.2));
  yield* all(codeLines[6].opacity(1, 0.12), codeLines[6].text('            rooms += 1', 0.2));
  yield* all(codeLines[7].opacity(1, 0.12), codeLines[7].text('        else:', 0.2));
  yield* all(codeLines[8].opacity(1, 0.12), codeLines[8].text('            ep += 1', 0.2));
  yield* all(codeLines[9].opacity(1, 0.12), codeLines[9].text('    return rooms', 0.2));
