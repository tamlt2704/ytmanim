...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Car Fleet'} fill={ORANGE} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Cars Merging on Highway'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  view.add(<Txt ref={tip} text={'Sort by position, stack arrival times\nSlower car blocks faster ones 🚗'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('Sort by position descending', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Calculate arrival time for each car', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('If slower car ahead → fleet merges', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Count distinct fleets ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def carFleet(target, pos, speed):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    pairs = sorted(zip(pos, speed), reverse=True)', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('    stack = []', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('    for p, s in pairs:', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('        time = (target - p) / s', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('        if not stack or time > stack[-1]:', 0.2));
  yield* all(codeLines[6].opacity(1, 0.12), codeLines[6].text('            stack.append(time)', 0.2));
  yield* all(codeLines[7].opacity(1, 0.12), codeLines[7].text('    return len(stack)', 0.2));
