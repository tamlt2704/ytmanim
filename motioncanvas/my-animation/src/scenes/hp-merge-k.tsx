...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Merge K Lists'} fill={RED} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Merge K Sorted Linked Lists'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  const arrY = -200;
    view.add(
      <Rect ref={boxes} x={startX + 0 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'1→4→5'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
    view.add(
  ...
  view.add(<Txt ref={tip} text={'O(N log k) — heap always has k items\nN = total nodes across all lists 🔗'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('Min-heap of k list heads', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Pop smallest, push its .next', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('1→1→2→3→4→4→5→6', 0.3);
  yield* waitFor(0.4);
  ...
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def mergeKLists(lists):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    heap = []', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('    for i, l in enumerate(lists):', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('        if l: heappush(heap, (l.val, i, l))', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('    dummy = curr = ListNode()', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('    while heap:', 0.2));
  yield* all(codeLines[6].opacity(1, 0.12), codeLines[6].text('        val, i, node = heappop(heap)', 0.2));
  yield* all(codeLines[7].opacity(1, 0.12), codeLines[7].text('        curr.next = node; curr = curr.next', 0.2));
  yield* all(codeLines[8].opacity(1, 0.12), codeLines[8].text('        if node.next:', 0.2));
  yield* all(codeLines[9].opacity(1, 0.12), codeLines[9].text('            heappush(heap, (node.next.val, i, node.next))', 0.2));
