...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Ingress'} fill={PURPLE} fontFamily={CODE_FONT} fontSize={90} fontWeight={800} y={-500} opacity={0} />);
  view.add(<Txt ref={sub} text={'HTTP Routing Rules'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-390} opacity={0} />);
  ...
  view.add(
    <Rect ref={card} width={750} height={400} y={50} radius={24} fill={TERMINAL_BG} stroke={PURPLE} lineWidth={4} opacity={0} scale={0.8}>
      ...
      <Txt text={'Route /api → backend service\nRoute / → frontend service'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={30} y={50} textAlign={'center'} lineHeight={48} />
      <Txt text={'TLS termination + path-based routing'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={24} y={130} textAlign={'center'} />
    </Rect>
  );
