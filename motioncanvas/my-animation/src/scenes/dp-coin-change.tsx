...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Coin Change'} fill={ORANGE} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Min Coins for Amount'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  view.add(<Txt ref={tip} text={'O(amount × coins) bottom-up\nClassic unbounded knapsack variant 🪙'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('amount=11, coins=[1,2,5]', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('dp[i] = min coins to make amount i', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('dp[11] = min(dp[10]+1, dp[9]+1, dp[6]+1)', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('dp[11] = 3 (5+5+1) ✅', 0.3);
