...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Binary Search'} fill={GREEN} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Search Sorted Array'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  const arrY = -200;
    ...
    view.add(
      <Rect ref={boxes} x={startX + 5 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'12'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
  ...
  view.add(<Txt ref={tip} text={'O(log n) — halve search space\neach iteration 🔍'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('target=9, l=0 r=5 mid=2 → 3<9', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('l=3 r=5 mid=4 → 9==9 found!', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Return index 4 ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def search(nums, target):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    l, r = 0, len(nums) - 1', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('    while l <= r:', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('        mid = (l + r) // 2', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('        if nums[mid] == target:', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('            return mid', 0.2));
  yield* all(codeLines[6].opacity(1, 0.12), codeLines[6].text('        elif nums[mid] < target: l = mid+1', 0.2));
  yield* all(codeLines[7].opacity(1, 0.12), codeLines[7].text('        else: r = mid - 1', 0.2));
