...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Top K Frequent'} fill={ORANGE} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'K Most Common via Heap'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  view.add(<Txt ref={tip} text={'O(n log k) with heap\nOr O(n) with bucket sort 🏆'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('Count frequencies: {1:3, 2:2, 3:1}', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Min-heap of size k by frequency', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Push all, keep k largest freq', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('k=2 → [1, 2] ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def topKFrequent(nums, k):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    count = Counter(nums)', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('    return nlargest(k, count.keys(),', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('        key=count.get)', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('    # Or with heap:', 0.2));
  yield* all(codeLines[6].opacity(1, 0.12), codeLines[6].text('    # heap = [(freq, num) for num, freq', 0.2));
  yield* all(codeLines[7].opacity(1, 0.12), codeLines[7].text('    #         in count.items()]', 0.2));
  yield* all(codeLines[8].opacity(1, 0.12), codeLines[8].text('    # return [n for _,n in nlargest(k, heap)]', 0.2));
