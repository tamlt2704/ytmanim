...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Largest Rect'} fill={RED} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Largest Rectangle in Histogram'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  view.add(<Txt ref={tip} text={'O(n) monotonic stack\nAppend sentinel 0 to flush stack 📊'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('Monotonic increasing stack of indices', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('When bar decreases: pop and calc area', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('height=5, width=2 → area=10 ★', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Max area = 10 ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def largestRectangleArea(heights):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    stack, best = [], 0', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('    for i, h in enumerate(heights + [0]):', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('        while stack and heights[stack[-1]] > h:', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('            height = heights[stack.pop()]', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('            width = i if not stack else i-stack[-1]-1', 0.2));
  yield* all(codeLines[6].opacity(1, 0.12), codeLines[6].text('            best = max(best, height * width)', 0.2));
