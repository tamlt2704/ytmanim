...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Copy Random'} fill={ORANGE} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Deep Copy with Random Ptr'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  const arrY = -200;
    ...
    view.add(
      <Rect ref={boxes} x={startX + 1 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'13→'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
    view.add(
      <Rect ref={boxes} x={startX + 2 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'11→'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
    view.add(
      <Rect ref={boxes} x={startX + 3 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'10→'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
    view.add(
  ...
  view.add(<Txt ref={tip} text={'HashMap maps old→new nodes\nO(n) time, O(n) space 🗺️'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('Each node has .next AND .random', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Pass 1: clone nodes, map old→new', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Pass 2: wire .next and .random', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Deep copy done! ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def copyRandomList(head):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    old_to_new = {None: None}', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('    curr = head', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('    while curr:', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('        old_to_new[curr] = Node(curr.val)', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('        curr = curr.next', 0.2));
  yield* all(codeLines[6].opacity(1, 0.12), codeLines[6].text('    curr = head', 0.2));
  yield* all(codeLines[7].opacity(1, 0.12), codeLines[7].text('    while curr:', 0.2));
  yield* all(codeLines[8].opacity(1, 0.12), codeLines[8].text('        copy = old_to_new[curr]', 0.2));
  yield* all(codeLines[9].opacity(1, 0.12), codeLines[9].text('        copy.next = old_to_new[curr.next]', 0.2));
  yield* all(codeLines[10].opacity(1, 0.12), codeLines[10].text('        copy.random = old_to_new[curr.random]', 0.2));
