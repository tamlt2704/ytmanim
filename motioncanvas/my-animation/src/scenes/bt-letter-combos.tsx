...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Letter Combos'} fill={ORANGE} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Phone Number Letters'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  const arrY = -200;
    view.add(
      <Rect ref={boxes} x={startX + 0 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'2→abc'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
    view.add(
      <Rect ref={boxes} x={startX + 1 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'3→def'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
  ...
  view.add(<Txt ref={tip} text={'O(4^n) — each digit maps to 3-4 letters\nBuild string char by char 📱'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* waitFor(0.4);
  yield* statusTxt().text('ad, ae, af, bd, be, bf, cd, ce, cf', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('9 combinations ✅', 0.3);
  ...
  yield* all(codeLines[8].opacity(1, 0.12), codeLines[8].text('        for c in phone[digits[i]]:', 0.2));
  yield* all(codeLines[9].opacity(1, 0.12), codeLines[9].text('            bt(i + 1, curr + c)', 0.2));
