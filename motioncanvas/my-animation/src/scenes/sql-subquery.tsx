...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Subquery'} fill={ACCENT_COLOR} fontFamily={CODE_FONT} fontSize={100} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Query in a Query'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={48} y={-490} opacity={0} />);
  ...
  view.add(<Txt ref={tip} text={'Subqueries run first\nthen feed the outer query 🪆'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={30} y={550} textAlign={'center'} lineHeight={50} opacity={0} />);
  ...
  yield* all(lines[1].opacity(1, 0.3), lines[1].text('WHERE id IN (SELECT user_id', 0.4));
  yield* all(lines[2].opacity(1, 0.3), lines[2].text('FROM orders WHERE total > 100)', 0.4));
