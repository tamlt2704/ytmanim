...
export default makeScene2D(function* (view) {
  ...
  yield* all(lines[0].opacity(1, 0.3), lines[0].text('SELECT * FROM users|WHERE id IN (|  SELECT user_id FROM orders|  WHERE total > 100);', 0.4));
  yield* all(lines[1].opacity(1, 0.3), lines[1].text('Subqueries run first\nthen feed the outer query 🪆', 0.4));
  yield* all(lines[2].opacity(1, 0.3), lines[2].text('Can use in WHERE, FROM, SELECT', 0.4));
