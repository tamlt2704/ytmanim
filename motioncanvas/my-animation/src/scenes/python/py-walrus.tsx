...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={subtitle} text={'Walrus Operator'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  view.add(<Txt ref={tip} text={'Assign inside expressions\nNew in Python 3.8 🦭'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={30} y={550} textAlign={'center'} lineHeight={50} opacity={0} />);
  ...
  yield* all(lines[0].opacity(1, 0.3), lines[0].text('if (n := len(data)) > 10:', 0.5));
  yield* waitFor(0.3);
  yield* all(lines[1].opacity(1, 0.3), lines[1].text('Assign AND test in one line', 0.4));
