...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'enumerate'} fill={GREEN} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Index + Value'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  view.add(<Txt ref={tip} text={'No more manual\ncounter variables! 🔢'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={30} y={550} textAlign={'center'} lineHeight={50} opacity={0} />);
  ...
  yield* all(lines[0].opacity(1, 0.3), lines[0].text('for i, v in enumerate(lst):', 0.5));
  yield* waitFor(0.3);
  yield* all(lines[1].opacity(1, 0.3), lines[1].text('i=0 v=apple, i=1 v=banana', 0.4));
