...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'GROUP BY'} fill={ACCENT_COLOR} fontFamily={CODE_FONT} fontSize={100} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Aggregate Data'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={48} y={-490} opacity={0} />);
  ...
  view.add(<Txt ref={tip} text={'COUNT, SUM, AVG, MIN, MAX\naggregate functions 📊'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={30} y={550} textAlign={'center'} lineHeight={50} opacity={0} />);
  ...
  yield* all(lines[0].opacity(1, 0.3), lines[0].text('SELECT dept, COUNT(*)', 0.4));
  yield* all(lines[1].opacity(1, 0.3), lines[1].text('FROM employees', 0.4));
  yield* all(lines[2].opacity(1, 0.3), lines[2].text('GROUP BY dept;', 0.4));
