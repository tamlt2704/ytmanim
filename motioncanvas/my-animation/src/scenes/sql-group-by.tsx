...
export default makeScene2D(function* (view) {
  ...
  yield* all(lines[0].opacity(1, 0.3), lines[0].text('SELECT dept, COUNT(*) as total|FROM employees|GROUP BY dept;', 0.4));
  yield* all(lines[1].opacity(1, 0.3), lines[1].text('Groups rows and applies\naggregate functions 📊', 0.4));
  yield* all(lines[2].opacity(1, 0.3), lines[2].text('COUNT, SUM, AVG, MIN, MAX', 0.4));
