...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Eval RPN'} fill={ORANGE} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Reverse Polish Notation'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  view.add(<Txt ref={tip} text={'O(n) — numbers push, operators pop two\nResult is last item on stack 🧮'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('2 → push, 1 → push', 0.3);
  yield* waitFor(0.3);
  yield* statusTxt().text('+ → pop 1,2 → push 3', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('3 → push, * → pop 3,3 → push 9', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Result: 9 ✅', 0.3);
