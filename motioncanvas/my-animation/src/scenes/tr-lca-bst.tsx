...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'LCA of BST'} fill={ORANGE} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Lowest Common Ancestor'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  const arrY = -200;
    ...
    view.add(
      <Rect ref={boxes} x={startX + 2 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'0 4 7 9'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
  ...
  view.add(<Txt ref={tip} text={'O(h) — BST lets us choose direction\nSplit point = LCA 🎯'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('BST property: left < root < right', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('p=2, q=8: split at 6 → LCA=6', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Both < root → go left', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('LCA found! ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def lowestCommonAncestor(root, p, q):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    while root:', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('        if p.val < root.val > q.val:', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('            root = root.left', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('        elif p.val > root.val < q.val:', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('            root = root.right', 0.2));
  yield* all(codeLines[6].opacity(1, 0.12), codeLines[6].text('        else:', 0.2));
  yield* all(codeLines[7].opacity(1, 0.12), codeLines[7].text('            return root', 0.2));
