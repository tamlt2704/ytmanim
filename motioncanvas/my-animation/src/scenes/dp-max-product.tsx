...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Max Product'} fill={ORANGE} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Max Product Subarray'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  view.add(<Txt ref={tip} text={'O(n) — negative × negative = positive\nTrack min for potential flip ✖️'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('Track both max AND min (negatives flip!)', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('max=6 (2*3), min=-2', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('At -2: new_max=max(-2, -2*-2)=4', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Max product = 6 ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def maxProduct(nums):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    res = max(nums)', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('    curMin = curMax = 1', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('    for n in nums:', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('        vals = (n, n*curMax, n*curMin)', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('        curMax, curMin = max(vals), min(vals)', 0.2));
  yield* all(codeLines[6].opacity(1, 0.12), codeLines[6].text('        res = max(res, curMax)', 0.2));
  yield* all(codeLines[7].opacity(1, 0.12), codeLines[7].text('    return res', 0.2));
