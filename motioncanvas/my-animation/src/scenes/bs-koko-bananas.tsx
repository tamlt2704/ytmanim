...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Koko Bananas'} fill={ORANGE} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Min Eating Speed'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  view.add(<Txt ref={tip} text={'Binary search on answer space\nO(n·log(max)) 🍌'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('h=8 hours, search speed 1..11', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('speed=6 → ceil(3/6)+ceil(6/6)+...=6 ≤ 8', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('speed=4 → total=8 ≤ 8 ✓ try lower', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Min speed = 4 ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def minEatingSpeed(piles, h):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    l, r = 1, max(piles)', 0.2));
  ...
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('        hours = sum(ceil(p/mid) for p in piles)', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('        if hours <= h: r = mid', 0.2));
  yield* all(codeLines[6].opacity(1, 0.12), codeLines[6].text('        else: l = mid + 1', 0.2));
  yield* all(codeLines[7].opacity(1, 0.12), codeLines[7].text('    return l', 0.2));
