...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Gas Station'} fill={ORANGE} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Circular Route Start'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  view.add(<Txt ref={tip} text={'O(n) — if total feasible, unique start\nReset when running sum goes negative ⛽'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('gas=[1,2,3,4,5] cost=[3,4,5,1,2]', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('If total gas >= total cost → solvable', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Reset start when tank goes negative', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Start at index 3 ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def canCompleteCircuit(gas, cost):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    if sum(gas) < sum(cost): return -1', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('    tank = start = 0', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('    for i in range(len(gas)):', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('        tank += gas[i] - cost[i]', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('        if tank < 0:', 0.2));
  yield* all(codeLines[6].opacity(1, 0.12), codeLines[6].text('            tank = 0', 0.2));
  yield* all(codeLines[7].opacity(1, 0.12), codeLines[7].text('            start = i + 1', 0.2));
  yield* all(codeLines[8].opacity(1, 0.12), codeLines[8].text('    return start', 0.2));
