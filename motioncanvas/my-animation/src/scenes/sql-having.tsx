...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={tip} text={'WHERE filters rows\nHAVING filters groups 🎯'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={30} y={550} textAlign={'center'} lineHeight={50} opacity={0} />);
  ...
  yield* all(lines[0].opacity(1, 0.3), lines[0].text('SELECT dept, AVG(salary)', 0.4));
  yield* all(lines[1].opacity(1, 0.3), lines[1].text('FROM employees GROUP BY dept', 0.4));
  yield* all(lines[2].opacity(1, 0.3), lines[2].text('HAVING AVG(salary) > 80000;', 0.4));
