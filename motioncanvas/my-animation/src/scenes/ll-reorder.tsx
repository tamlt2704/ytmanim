...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Reorder List'} fill={ORANGE} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'L0→Ln→L1→Ln-1→...'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  yield* statusTxt().text('1. Find middle with slow/fast', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('2. Reverse second half: 5→4→3', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('3. Merge alternating: 1→5→2→4→3', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Reordered! ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def reorderList(head):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    slow = fast = head', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('    while fast and fast.next:', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('        slow, fast = slow.next, fast.next.next', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('    # reverse second half from slow', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('    prev, curr = None, slow.next', 0.2));
  yield* all(codeLines[6].opacity(1, 0.12), codeLines[6].text('    slow.next = None', 0.2));
  yield* all(codeLines[7].opacity(1, 0.12), codeLines[7].text('    # merge two halves alternating', 0.2));
