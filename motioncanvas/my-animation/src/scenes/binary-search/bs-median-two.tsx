...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Median Two'} fill={RED} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Median of 2 Sorted Arrays'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  const arrY = -200;
    ...
    view.add(
      <Rect ref={boxes} x={startX + 3 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'9'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
    view.add(
      <Rect ref={boxes} x={startX + 4 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'15'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
  ...
  view.add(<Txt ref={tip} text={'O(log(min(m,n))) — binary search\non partition of smaller array 🧠'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('Binary search on smaller array', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Partition both arrays at midpoint', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Left maxes ≤ right mins → valid!', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('O(log(min(m,n))) ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def findMedianSortedArrays(a, b):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    if len(a) > len(b): a, b = b, a', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('    m, n = len(a), len(b)', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('    l, r = 0, m', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('    while l <= r:', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('        i = (l + r) // 2', 0.2));
  yield* all(codeLines[6].opacity(1, 0.12), codeLines[6].text('        j = (m+n+1)//2 - i', 0.2));
  yield* all(codeLines[7].opacity(1, 0.12), codeLines[7].text('        # check cross boundaries', 0.2));
  yield* all(codeLines[8].opacity(1, 0.12), codeLines[8].text('        # adjust l or r accordingly', 0.2));
