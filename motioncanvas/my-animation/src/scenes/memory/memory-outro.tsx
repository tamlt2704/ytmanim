...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={recap} text={'10 tricks to remember anything!'} fill={ACCENT_COLOR} fontFamily={CODE_FONT} fontSize={34} fontWeight={700} y={130} opacity={0} />);

  const topics = createRef<Txt>();
  view.add(<Txt ref={topics} text={'🧩 Chunking  🏠 Memory Palace\n🔁 Spaced Rep  🔤 Acronyms\n📖 Story  🎨 Visual  ❓ Recall\n😴 Sleep  🧪 Teach  📝+🖼️ Dual'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={24} y={310} textAlign={'center'} lineHeight={42} opacity={0} />);

  const follow = createRef<Txt>();
  view.add(<Txt ref={follow} text={'More brain hacks coming! 🔥'} fill={ORANGE} fontFamily={TITLE_FONT} fontSize={36} y={500} opacity={0} />);

  const handle = createRef<Txt>();
  view.add(<Txt ref={handle} text={'@it4life101'} fill={PURPLE} fontFamily={CODE_FONT} fontSize={44} fontWeight={800} y={580} opacity={0} />);
