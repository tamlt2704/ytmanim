...
import {BG_COLOR, TEXT_COLOR, ACCENT_COLOR, GREEN, RED, TERMINAL_BG, CODE_FONT, TITLE_FONT, ORANGE, PURPLE} from '../../styles';
...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'ciw'} fill={PURPLE} fontFamily={CODE_FONT} fontSize={120} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Change Inner Word'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={48} y={-490} opacity={0} />);

  const cmds = createRefArray<Rect>();
  const cmdData = [
    {cmd: 'ciw', desc: 'Change inner word', color: PURPLE, y: -220},
    {cmd: 'ci"', desc: 'Change inside quotes', color: PURPLE, y: -70},
    {cmd: 'ci(', desc: 'Change inside parens', color: ORANGE, y: 80},
    {cmd: 'cit', desc: 'Change inside HTML tag', color: ORANGE, y: 230},
  ];
  ...
  view.add(<Txt ref={tip} text={'c = change, i = inner\nThe most powerful Vim combo ⚡'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={30} y={550} textAlign={'center'} lineHeight={50} opacity={0} />);
