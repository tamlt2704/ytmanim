...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Min Rotated'} fill={ORANGE} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Find Min in Rotated Array'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  view.add(<Txt ref={tip} text={'O(log n) — compare mid vs right\nto find rotation point 🔄'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('Sorted then rotated at some pivot', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('mid=5 > right=2 → min is right half', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('mid=1 < right=2 → min is left half', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Minimum = 1 ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def findMin(nums):', 0.2));
  ...
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('        if nums[mid] > nums[r]:', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('            l = mid + 1', 0.2));
  yield* all(codeLines[6].opacity(1, 0.12), codeLines[6].text('        else:', 0.2));
  yield* all(codeLines[7].opacity(1, 0.12), codeLines[7].text('            r = mid', 0.2));
