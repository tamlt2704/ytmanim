...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={recap} text={'10 Google tricks you needed!'} fill={ACCENT_COLOR} fontFamily={CODE_FONT} fontSize={32} fontWeight={700} y={120} opacity={0} />);

  const topics = createRef<Txt>();
  view.add(<Txt ref={topics} text={'"quotes"  -exclude  site:\nfiletype:  before:/after:  OR\n*wildcard  intitle:  related:\n🔥 Combo Power'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={24} y={280} textAlign={'center'} lineHeight={42} opacity={0} />);

  const follow = createRef<Txt>();
  view.add(<Txt ref={follow} text={'More tech tricks coming! 🔥'} fill={ORANGE} fontFamily={TITLE_FONT} fontSize={36} y={450} opacity={0} />);

  const handle = createRef<Txt>();
  view.add(<Txt ref={handle} text={'@it4life101'} fill={PURPLE} fontFamily={CODE_FONT} fontSize={44} fontWeight={800} y={530} opacity={0} />);
