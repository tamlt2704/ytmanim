...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Search Rotated'} fill={ORANGE} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Find Target in Rotated'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  view.add(<Txt ref={tip} text={'O(log n) — determine which half\nis sorted, then narrow down 🎯'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('target=0, mid=7, left sorted [4,5,6,7]', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('0 not in [4..7] → search right half', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('mid=1, right sorted [0,1,2] → found!', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Index 4 ✅', 0.3);
  ...
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('        if nums[mid] == target: return mid', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('        if nums[l] <= nums[mid]:', 0.2));
  yield* all(codeLines[6].opacity(1, 0.12), codeLines[6].text('            if nums[l]<=target<nums[mid]: r=mid-1', 0.2));
  yield* all(codeLines[7].opacity(1, 0.12), codeLines[7].text('            else: l = mid + 1', 0.2));
