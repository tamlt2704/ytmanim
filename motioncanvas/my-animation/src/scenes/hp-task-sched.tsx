...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Task Scheduler'} fill={ORANGE} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Min Intervals with Cooldown'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  view.add(<Txt ref={tip} text={'Heap + cooldown queue\nGreedy: always run most frequent 🗓️'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('n=2 cooldown between same tasks', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Max-heap by frequency: A=3, B=3', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('A B idle A B idle A B', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Total intervals = 8 ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def leastInterval(tasks, n):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    count = Counter(tasks)', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('    heap = [-c for c in count.values()]', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('    heapify(heap); time = 0; q = deque()', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('    while heap or q:', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('        time += 1', 0.2));
  yield* all(codeLines[6].opacity(1, 0.12), codeLines[6].text('        if heap:', 0.2));
  yield* all(codeLines[7].opacity(1, 0.12), codeLines[7].text('            cnt = 1 + heappop(heap)', 0.2));
  yield* all(codeLines[8].opacity(1, 0.12), codeLines[8].text('            if cnt: q.append((cnt, time + n))', 0.2));
  yield* all(codeLines[9].opacity(1, 0.12), codeLines[9].text('        if q and q[0][1] == time:', 0.2));
  yield* all(codeLines[10].opacity(1, 0.12), codeLines[10].text('            heappush(heap, q.popleft()[0])', 0.2));
