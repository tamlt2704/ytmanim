...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Missing Number'} fill={GREEN} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Find Missing in 0..n'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  view.add(<Txt ref={tip} text={'O(n) time, O(1) space\nXOR index with value, missing survives 🔍'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('XOR all nums with 0..n', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Pairs cancel: 0^0=0, 1^1=0, 3^3=0', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Only missing number 2 remains', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Missing = 2 ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def missingNumber(nums):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    res = len(nums)', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('    for i, n in enumerate(nums):', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('        res ^= i ^ n', 0.2));
