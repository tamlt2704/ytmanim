...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Sum Two Ints'} fill={ORANGE} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Add Without + Operator'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  const arrY = -200;
    view.add(
      <Rect ref={boxes} x={startX + 0 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'a=5'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
    view.add(
      <Rect ref={boxes} x={startX + 1 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'b=3'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
    view.add(
  ...
  view.add(<Txt ref={tip} text={'XOR = add, AND<<1 = carry\nMask handles Python negative ints ➕'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('XOR = sum without carry', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('AND << 1 = carry bits', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Repeat until no carry', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('5 + 3 = 8 ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def getSum(a, b):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    mask = 0xFFFFFFFF', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('    while b & mask:', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('        carry = (a & b) << 1', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('        a = a ^ b', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('        b = carry', 0.2));
  yield* all(codeLines[6].opacity(1, 0.12), codeLines[6].text('    return a & mask if b > 0 else a', 0.2));
