...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Reverse Bits'} fill={GREEN} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Reverse 32-bit Integer'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  const arrY = -200;
    view.add(
      <Rect ref={boxes} x={startX + 0 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'1→0'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
    view.add(
      <Rect ref={boxes} x={startX + 1 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'0→1'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
    view.add(
    ...
    view.add(
      <Rect ref={boxes} x={startX + 3 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'...32 bits'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
  ...
  view.add(<Txt ref={tip} text={'O(32) = O(1) — shift bit by bit\nBuild result from LSB to MSB 🔄'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('Extract last bit, shift into result', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('res = (res << 1) | (n & 1)', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('n >>= 1, repeat 32 times', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Reversed! ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def reverseBits(n):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    res = 0', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('    for _ in range(32):', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('        res = (res << 1) | (n & 1)', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('        n >>= 1', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('    return res', 0.2));
