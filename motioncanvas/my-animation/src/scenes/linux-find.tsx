...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'find'} fill={GREEN} fontFamily={CODE_FONT} fontSize={120} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Locate Files & Dirs'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={48} y={-490} opacity={0} />);
  ...
  view.add(<Txt ref={tip} text={'Combine with -exec to run\ncommands on results ⚡'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={30} y={550} textAlign={'center'} lineHeight={50} opacity={0} />);
  ...
  const cmd = '$ find . -name "*.log" -mtime -7';
  for (let i = 0; i <= cmd.length; i++) { lines[0].text(cmd.slice(0, i) + '▌'); yield* waitFor(0.04); }
  lines[0].text(cmd);
  yield* waitFor(0.3);
  yield* all(lines[1].opacity(1, 0.3), lines[1].text('./logs/app.log', 0.3));
  yield* all(lines[2].opacity(1, 0.3), lines[2].text('./logs/error.log', 0.3));
  yield* all(lines[3].opacity(1, 0.3), lines[3].text('./tmp/debug.log', 0.3));
