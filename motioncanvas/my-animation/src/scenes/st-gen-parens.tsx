...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Gen Parens'} fill={ORANGE} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Generate All Valid Combos'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  view.add(<Txt ref={tip} text={'Backtracking with open/close counts\nOnly add ) when close < open 🌿'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('Backtrack: track open and close count', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Add ( if open < n', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Add ) if close < open', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('n=3 → 5 combinations ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def generateParenthesis(n):', 0.2));
