...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={name} text={'Tim Cook'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={72} fontWeight={900} y={-600} opacity={0} />);
  yield* fadeIn(name(), 0.3);

  const company = createRef<Txt>();
  view.add(<Txt ref={company} text={'CEO of Apple'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={32} y={-530} opacity={0} />);
  ...
  view.add(
    <Rect ref={annual} width={440} height={80} radius={18} fill={'#21262d'} y={-410} opacity={0}>
      <Txt text={'Annual: $98.7 million'} fill={GREEN} fontFamily={CODE_FONT} fontSize={30} fontWeight={800} />
    </Rect>,
  );
  ...
  const steps = ['$0.50', '$1.00', '$1.80', '$2.40', '$2.80', '$3.00', '$3.13'];
  ...
  view.add(<Txt ref={live} text={'$0'} fill={GREEN} fontFamily={CODE_FONT} fontSize={72} fontWeight={900} y={0} opacity={0} />);
  yield* live().opacity(1, 0.2);

  // Simulate ~10 seconds of earning
  for (let i = 1; i <= 10; i++) {
    live().text(`$${(i * 3.13).toFixed(2)}`);
    yield* waitFor(0.3);
  }

  const context = createRef<Txt>();
  view.add(<Txt ref={context} text={'He just made $31 while\nyou watched this 💸'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={26} y={120} textAlign={'center'} lineHeight={40} opacity={0} />);
