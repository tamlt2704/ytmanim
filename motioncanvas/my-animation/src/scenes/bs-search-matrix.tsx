...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Search Matrix'} fill={ORANGE} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Sorted 2D Matrix'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  const arrY = -200;
    ...
    view.add(
      <Rect ref={boxes} x={startX + 0 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'[1,3,5]'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
    view.add(
      <Rect ref={boxes} x={startX + 1 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'[10,11,16]'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
    view.add(
      <Rect ref={boxes} x={startX + 2 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'[23,30,34]'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
  ...
  view.add(<Txt ref={tip} text={'O(log(m·n)) — flatten indices\nrow = mid//cols, col = mid%cols 🧮'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('Treat matrix as flat sorted array', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('mid=4 → row=4//3=1, col=4%3=1 → 11', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Binary search on virtual 1D array', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('O(log(m*n)) ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def searchMatrix(matrix, target):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    m, n = len(matrix), len(matrix[0])', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('    l, r = 0, m * n - 1', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('    while l <= r:', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('        mid = (l + r) // 2', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('        val = matrix[mid//n][mid%n]', 0.2));
  yield* all(codeLines[6].opacity(1, 0.12), codeLines[6].text('        if val == target: return True', 0.2));
  yield* all(codeLines[7].opacity(1, 0.12), codeLines[7].text('        elif val < target: l = mid+1', 0.2));
  yield* all(codeLines[8].opacity(1, 0.12), codeLines[8].text('        else: r = mid - 1', 0.2));
