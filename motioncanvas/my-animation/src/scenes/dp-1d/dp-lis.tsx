...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'LIS'} fill={ORANGE} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Longest Increasing Subseq'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  const arrY = -200;
    view.add(
      <Rect ref={boxes} x={startX + 0 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'10'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={28} fontWeight={800} />
      </Rect>
    );
    view.add(
      <Rect ref={boxes} x={startX + 1 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'9'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={28} fontWeight={800} />
      </Rect>
    );
    view.add(
    ...
    view.add(
      <Rect ref={boxes} x={startX + 6 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'101'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={28} fontWeight={800} />
      </Rect>
    );
    view.add(
      <Rect ref={boxes} x={startX + 7 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'18'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={28} fontWeight={800} />
      </Rect>
    );
  ...
  view.add(
    <Rect ref={codeBox} width={900} height={298} y={320} radius={16} fill={TERMINAL_BG} stroke={TERMINAL_BORDER} lineWidth={2} opacity={0} scale={0.8} clip>
      <Rect width={900} height={44} y={-127} fill={'#21262d'}><Txt text={'Python'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={18} /></Rect>
  ...
  view.add(<Txt ref={tip} text={'O(n²) DP or O(n log n) with bisect\nPatience sorting approach 📈'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('dp[i] = LIS ending at index i', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('For each j < i: if nums[j] < nums[i]', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('dp[i] = max(dp[i], dp[j] + 1)', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('LIS = 4 → [2,3,7,101] ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def lengthOfLIS(nums):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    dp = [1] * len(nums)', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('    for i in range(1, len(nums)):', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('        for j in range(i):', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('            if nums[j] < nums[i]:', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('                dp[i] = max(dp[i], dp[j]+1)', 0.2));
  yield* all(codeLines[6].opacity(1, 0.12), codeLines[6].text('    return max(dp)', 0.2));
