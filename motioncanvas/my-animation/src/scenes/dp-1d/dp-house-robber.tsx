...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'House Robber'} fill={ORANGE} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Max Loot, No Adjacent'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  view.add(<Txt ref={tip} text={'O(n) time, O(1) space\nRob current + prev2 vs skip 🏠'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('Rob or skip each house', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('dp[i] = max(dp[i-1], dp[i-2]+nums[i])', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('dp = [1, 2, 4, 4]', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Max loot = 4 (houses 0+2) ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def rob(nums):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    prev2, prev1 = 0, 0', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('    for n in nums:', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('        curr = max(prev1, prev2 + n)', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('        prev2, prev1 = prev1, curr', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('    return prev1', 0.2));
