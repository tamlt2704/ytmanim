...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'zip()'} fill={GREEN} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Pair Up Iterables'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  view.add(<Txt ref={tip} text={'Combines multiple lists\nelement by element 🤝'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={30} y={550} textAlign={'center'} lineHeight={50} opacity={0} />);
  ...
  yield* all(lines[0].opacity(1, 0.3), lines[0].text('list(zip(names, scores))', 0.5));
  yield* waitFor(0.3);
  yield* all(lines[1].opacity(1, 0.3), lines[1].text("[('Alice',95), ('Bob',87)]", 0.4));
