...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Dict Comp'} fill={GREEN} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Build Dicts Fast'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  view.add(<Txt ref={tip} text={'Same idea as list comp\nbut for dictionaries 📖'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={30} y={550} textAlign={'center'} lineHeight={50} opacity={0} />);
  ...
  yield* all(lines[0].opacity(1, 0.3), lines[0].text('{k: v for k, v in pairs}', 0.5));
  yield* waitFor(0.3);
  yield* all(lines[1].opacity(1, 0.3), lines[1].text('{'a': 1, 'b': 2}', 0.4));
