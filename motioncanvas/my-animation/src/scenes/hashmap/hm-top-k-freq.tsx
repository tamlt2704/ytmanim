...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={subtitle} text={'K Most Common Elements'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  view.add(<Txt ref={tip} text={'O(n) with bucket sort\nAvoids O(n log n) heap approach 🪣'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('Count: {1:3, 2:2, 3:1}', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Bucket sort by frequency', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('freq[3]=[1], freq[2]=[2], freq[1]=[3]', 0.3);
  yield* waitFor(0.4);
  ...
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('    bucket = [[] for _ in range(len(nums)+1)]', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('    for n, c in count.items():', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('        bucket[c].append(n)', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('    res = []', 0.2));
  yield* all(codeLines[6].opacity(1, 0.12), codeLines[6].text('    for i in range(len(bucket)-1, -1, -1):', 0.2));
  yield* all(codeLines[7].opacity(1, 0.12), codeLines[7].text('        res.extend(bucket[i])', 0.2));
  yield* all(codeLines[8].opacity(1, 0.12), codeLines[8].text('        if len(res) >= k: return res[:k]', 0.2));
