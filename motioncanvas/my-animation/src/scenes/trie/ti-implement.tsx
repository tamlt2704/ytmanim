...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Implement Trie'} fill={ORANGE} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Prefix Tree Structure'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  const arrY = -200;
    view.add(
      <Rect ref={boxes} x={startX + 0 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'root'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
    view.add(
      <Rect ref={boxes} x={startX + 1 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'→a→p→p'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
    view.add(
      <Rect ref={boxes} x={startX + 2 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'→a→p→e'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
  ...
  view.add(
    <Rect ref={codeBox} width={900} height={490} y={320} radius={16} fill={TERMINAL_BG} stroke={TERMINAL_BORDER} lineWidth={2} opacity={0} scale={0.8} clip>
      ...
      <Txt ref={codeLines} text={''} fill={PURPLE} fontFamily={CODE_FONT} fontSize={20} x={-410} y={82} offsetX={-1} opacity={0} />
      ...
      <Txt ref={codeLines} text={''} fill={GREEN} fontFamily={CODE_FONT} fontSize={20} x={-410} y={242} offsetX={-1} opacity={0} />
      <Txt ref={codeLines} text={''} fill={GREEN} fontFamily={CODE_FONT} fontSize={20} x={-410} y={274} offsetX={-1} opacity={0} />
    </Rect>
  );
  const tip = createRef<Txt>();
  view.add(<Txt ref={tip} text={'O(L) insert/search per word\nShared prefixes save space 🌳'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('Each node = dict of children + end flag', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('insert: walk/create nodes, mark end', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('search: walk nodes, check end flag', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('startsWith: walk, no end check ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('class TrieNode:', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    def __init__(self):', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('        self.children = {}', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('        self.end = False', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('class Trie:', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('    def __init__(self): self.root = TrieNode()', 0.2));
  yield* all(codeLines[6].opacity(1, 0.12), codeLines[6].text('    def insert(self, word):', 0.2));
  yield* all(codeLines[7].opacity(1, 0.12), codeLines[7].text('        node = self.root', 0.2));
  yield* all(codeLines[8].opacity(1, 0.12), codeLines[8].text('        for c in word:', 0.2));
  yield* all(codeLines[9].opacity(1, 0.12), codeLines[9].text('            if c not in node.children:', 0.2));
  yield* all(codeLines[10].opacity(1, 0.12), codeLines[10].text('                node.children[c] = TrieNode()', 0.2));
  yield* all(codeLines[11].opacity(1, 0.12), codeLines[11].text('            node = node.children[c]', 0.2));
  yield* all(codeLines[12].opacity(1, 0.12), codeLines[12].text('        node.end = True', 0.2));
