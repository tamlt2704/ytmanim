...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Merge Triplets'} fill={ORANGE} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Can Triplets Form Target?'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  const arrY = -200;
    view.add(
      <Rect ref={boxes} x={startX + 0 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'[2,5,3]'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
    view.add(
      <Rect ref={boxes} x={startX + 1 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'[1,8,4]'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
    view.add(
      <Rect ref={boxes} x={startX + 2 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'[1,7,5]'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
  ...
  view.add(<Txt ref={tip} text={'O(n) — filter invalid triplets\nCheck if all 3 positions covered 🎯'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('target=[2,7,5]', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Skip triplets with any val > target', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('From valid ones, can we cover each pos?', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Yes → True ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def mergeTriplets(triplets, target):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    good = set()', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('    for t in triplets:', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('        if (t[0] > target[0] or', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('            t[1] > target[1] or', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('            t[2] > target[2]):', 0.2));
  yield* all(codeLines[6].opacity(1, 0.12), codeLines[6].text('            continue', 0.2));
  yield* all(codeLines[7].opacity(1, 0.12), codeLines[7].text('        for i in range(3):', 0.2));
  yield* all(codeLines[8].opacity(1, 0.12), codeLines[8].text('            if t[i] == target[i]: good.add(i)', 0.2));
  yield* all(codeLines[9].opacity(1, 0.12), codeLines[9].text('    return len(good) == 3', 0.2));
