...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Jump Game II'} fill={ORANGE} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Min Jumps to Reach End'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  view.add(<Txt ref={tip} text={'O(n) — implicit BFS with levels\nJump when reaching current boundary 🦘'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('BFS-like: expand level by level', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Jump 1: from 0, reach [1,2]', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Jump 2: from [1,2], reach [4] → end!', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Min jumps = 2 ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def jump(nums):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    jumps = end = farthest = 0', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('    for i in range(len(nums) - 1):', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('        farthest = max(farthest, i + nums[i])', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('        if i == end:', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('            jumps += 1', 0.2));
  yield* all(codeLines[6].opacity(1, 0.12), codeLines[6].text('            end = farthest', 0.2));
  yield* all(codeLines[7].opacity(1, 0.12), codeLines[7].text('    return jumps', 0.2));
