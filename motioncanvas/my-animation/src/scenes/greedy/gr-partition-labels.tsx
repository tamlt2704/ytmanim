...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Partition Labels'} fill={ORANGE} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Max Partitions, Unique Chars'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  const boxSize = 52; const gap = 4;
  const startX = -((12 - 1) * (boxSize + gap)) / 2;
  const arrY = -200;
    view.add(
      <Rect ref={boxes} x={startX + 0 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'a'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={22} fontWeight={800} />
      </Rect>
    );
    view.add(
      <Rect ref={boxes} x={startX + 1 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'b'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={22} fontWeight={800} />
      </Rect>
    );
    view.add(
    ...
    view.add(
      <Rect ref={boxes} x={startX + 4 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'c'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={22} fontWeight={800} />
      </Rect>
    );
    view.add(
    ...
    view.add(
      <Rect ref={boxes} x={startX + 9 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'d'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={22} fontWeight={800} />
      </Rect>
    );
    view.add(
      <Rect ref={boxes} x={startX + 10 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'e'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={22} fontWeight={800} />
      </Rect>
    );
    view.add(
      <Rect ref={boxes} x={startX + 11 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'f'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={22} fontWeight={800} />
      </Rect>
    );
  ...
  view.add(<Txt ref={tip} text={'O(n) — extend end to last[char]\nCut partition when i reaches end ✂️'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('Each char appears in at most one part', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Track last occurrence of each char', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Extend partition to cover all chars in it', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('[9, 2, 1] → sizes ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def partitionLabels(s):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    last = {c: i for i, c in enumerate(s)}', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('    res, start, end = [], 0, 0', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('    for i, c in enumerate(s):', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('        end = max(end, last[c])', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('        if i == end:', 0.2));
  yield* all(codeLines[6].opacity(1, 0.12), codeLines[6].text('            res.append(end - start + 1)', 0.2));
  yield* all(codeLines[7].opacity(1, 0.12), codeLines[7].text('            start = i + 1', 0.2));
