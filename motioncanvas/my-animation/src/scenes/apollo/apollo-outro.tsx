...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={recap} text={'📱 > 🚀 in every spec!'} fill={ACCENT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={700} y={130} opacity={0} />);

  const topics = createRef<Txt>();
  view.add(<Txt ref={topics} text={'RAM · CPU · Storage\nWeight · Power · Price'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={28} y={260} textAlign={'center'} lineHeight={44} opacity={0} />);

  const follow = createRef<Txt>();
  view.add(<Txt ref={follow} text={'More mind-blowing comparisons! 🔥'} fill={ORANGE} fontFamily={TITLE_FONT} fontSize={34} y={400} opacity={0} />);

  const handle = createRef<Txt>();
  view.add(<Txt ref={handle} text={'@it4life101'} fill={PURPLE} fontFamily={CODE_FONT} fontSize={44} fontWeight={800} y={490} opacity={0} />);
