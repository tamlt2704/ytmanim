...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'K Closest'} fill={ORANGE} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'K Nearest Points to Origin'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  const arrY = -200;
    ...
    view.add(
      <Rect ref={boxes} x={startX + 1 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'(-2,2)'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
    view.add(
      <Rect ref={boxes} x={startX + 2 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'(3,1)'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
    view.add(
  ...
  view.add(<Txt ref={tip} text={'Max-heap of size k keeps k smallest\nO(n log k) time 📍'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('Distance = x² + y² (skip sqrt)', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Max-heap of size k=2', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('Keep k smallest distances', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('[(-2,2),(1,3)] closest ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def kClosest(points, k):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    heap = []', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('    for x, y in points:', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('        dist = x*x + y*y', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('        heappush(heap, (-dist, x, y))', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('        if len(heap) > k:', 0.2));
  yield* all(codeLines[6].opacity(1, 0.12), codeLines[6].text('            heappop(heap)', 0.2));
  yield* all(codeLines[7].opacity(1, 0.12), codeLines[7].text('    return [[x,y] for _,x,y in heap]', 0.2));
