...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Kth Largest'} fill={GREEN} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Stream of Numbers'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  const arrY = -200;
    ...
    view.add(
      <Rect ref={boxes} x={startX + 1 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'5'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
    view.add(
  ...
  view.add(<Txt ref={tip} text={'Min-heap of size k\nTop = kth largest element 📊'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('Min-heap of size k=3', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('heap = [4, 5, 8] → kth = 4', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('add(3): heap stays [4,5,8] → 4', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('add(5): heap=[5,5,8] → kth=5 ✅', 0.3);
  yield* statusTxt().fill(GREEN, 0.2);
  yield* waitFor(0.5);

  yield* all(codeBox().opacity(1, 0.4), codeBox().scale(1, 0.5, easeOutCubic));
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('class KthLargest:', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    def __init__(self, k, nums):', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('        self.k = k', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('        self.heap = nums', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('        heapify(self.heap)', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('        while len(self.heap) > k:', 0.2));
  yield* all(codeLines[6].opacity(1, 0.12), codeLines[6].text('            heappop(self.heap)', 0.2));
  yield* all(codeLines[7].opacity(1, 0.12), codeLines[7].text('    def add(self, val):', 0.2));
  yield* all(codeLines[8].opacity(1, 0.12), codeLines[8].text('        heappush(self.heap, val)', 0.2));
  yield* all(codeLines[9].opacity(1, 0.12), codeLines[9].text('        if len(self.heap) > self.k:', 0.2));
  yield* all(codeLines[10].opacity(1, 0.12), codeLines[10].text('            heappop(self.heap)', 0.2));
  yield* all(codeLines[11].opacity(1, 0.12), codeLines[11].text('        return self.heap[0]', 0.2));
