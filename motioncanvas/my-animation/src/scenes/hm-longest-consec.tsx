...
export default makeScene2D(function* (view) {
  ...
  view.add(<Txt ref={title} text={'Longest Consec'} fill={ORANGE} fontFamily={CODE_FONT} fontSize={80} fontWeight={800} y={-580} opacity={0} />);
  view.add(<Txt ref={subtitle} text={'Longest Consecutive Run'} fill={TEXT_COLOR} fontFamily={TITLE_FONT} fontSize={44} y={-490} opacity={0} />);
  ...
  const arrY = -200;
    ...
    view.add(
      <Rect ref={boxes} x={startX + 0 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'100'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
    view.add(
    ...
    view.add(
      <Rect ref={boxes} x={startX + 2 * (boxSize + gap)} y={arrY} width={boxSize} height={boxSize} radius={12} fill={TERMINAL_BG} stroke={'#30363d'} lineWidth={2} opacity={0} scale={0}>
        <Txt text={'200'} fill={TEXT_COLOR} fontFamily={CODE_FONT} fontSize={36} fontWeight={800} />
      </Rect>
    );
    view.add(
  ...
  view.add(<Txt ref={tip} text={'O(n) — only start counting\nfrom sequence beginnings 🔗'} fill={'#8b949e'} fontFamily={CODE_FONT} fontSize={28} y={640} textAlign={'center'} lineHeight={46} opacity={0} />);
  ...
  yield* statusTxt().text('Set: {100, 4, 200, 1, 3, 2}', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('1 has no left neighbor → start!', 0.3);
  yield* waitFor(0.4);
  yield* statusTxt().text('1→2→3→4 = length 4 ★', 0.3);
  yield* waitFor(0.4);
  ...
  yield* all(codeLines[0].opacity(1, 0.12), codeLines[0].text('def longestConsecutive(nums):', 0.2));
  yield* all(codeLines[1].opacity(1, 0.12), codeLines[1].text('    s = set(nums); best = 0', 0.2));
  yield* all(codeLines[2].opacity(1, 0.12), codeLines[2].text('    for n in s:', 0.2));
  yield* all(codeLines[3].opacity(1, 0.12), codeLines[3].text('        if n - 1 not in s:', 0.2));
  yield* all(codeLines[4].opacity(1, 0.12), codeLines[4].text('            length = 1', 0.2));
  yield* all(codeLines[5].opacity(1, 0.12), codeLines[5].text('            while n + length in s:', 0.2));
  yield* all(codeLines[6].opacity(1, 0.12), codeLines[6].text('                length += 1', 0.2));
  yield* all(codeLines[7].opacity(1, 0.12), codeLines[7].text('            best = max(best, length)', 0.2));
